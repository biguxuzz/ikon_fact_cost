...
| 2 | Разделить MCP-инструменты prod_cost / prod_cost_ext | Средний | 📋 Описано ниже |
...
## Задача 2: Разделение алгоритмов по отчётам (РЕКОМЕНДУЕТСЯ)

### Цель

Обеспечить автоматический выбор алгоритма **без передачи дополнительных параметров**:

| Отчёт | Алгоритм | Поведение |
|-------|----------|-----------|
| **Типовой** `ФактическаяСебестоимостьПродукции` | **Старый** (рекурсивный, эталонный) | По умолчанию |
| **Новый** `ikon_cost_ФактическаяСебестоимостьПродукции` | **Новый** (итеративный по уровням) | Явно устанавливается в модуле отчёта |
...
**Файл**: `Reports\ikon_cost_ФактическаяСебестоимостьПродукции\Ext\ObjectModule.bsl`, строка 136:
```
...
**Принцип**: Значение по умолчанию — старый алгоритм. Новый отчёт явно включает новый алгоритм.

#### Этап 2.1: Изменить значение по умолчанию в расширении
...
| Было | Стало |
|------|-------|
| `ПараметрыДерева.Вставить("ИспользоватьПакетнуюОбработку", Истина);` | `ПараметрыДерева.Вставить("ИспользоватьПакетнуюОбработку", Ложь);` |

**Результат**: Все типовые отчёты (и любой код, использующий `ПараметрыДереваСебестоимости()`) будут работать со **старым алгоритмом** по умолчанию.

#### Этап 2.2: Включить новый алгоритм в новом отчёте

**Место**: `Reports\ikon_cost_ФактическаяСебестоимостьПродукции\Ext\ObjectModule.bsl`

**Изменение**: После строки 136 добавить переопределение параметра:

| Строка | Содержимое |
|--------|------------|
| 136 | `ПараметрыДереваСебестоимости = СтруктураСебестоимости.ПараметрыДереваСебестоимости();` |
| **137 (новая)** | `ПараметрыДереваСебестоимости.ИспользоватьПакетнуюОбработку = Истина;` |

**Результат**: Только новый отчёт `ikon_cost_ФактическаяСебестоимостьПродукции` будет использовать **новый алгоритм**.

### Интеграция с MCP-сервером

**На стороне MCP-сервера** (`gst_fresh_mcp`):

| Инструмент | Вызываемый отчёт | Алгоритм |
|------------|------------------|----------|
| `prod_cost` | Типовой `ФактическаяСебестоимостьПродукции` | Старый |
| `prod_cost_ext` | Новый `ikon_cost_ФактическаяСебестоимостьПродукции` | Новый |

**Изменения**: Создать инструмент `prod_cost_ext`, который вызывает новый отчёт вместо типового.
