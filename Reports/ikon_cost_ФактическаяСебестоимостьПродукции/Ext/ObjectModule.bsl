...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	// === КЭШИРОВАНИЕ: Проверка наличия данных за период ===
	...
	Если ЕстьКэш Тогда
		...
		// === ВАРИАНТ А: Берём данные из кэша (быстро) ===
		ДанныеИзКэша = ikon_cost_КэшированиеРасчётов.ПолучитьДанныеИзКэша(
			ПериодРасчёта,
			НастройкиОтчета,
		...
		ВнешниеНаборыДанных.Вставить("Факт", ДанныеИзКэша.Факт);
		ВнешниеНаборыДанных.Вставить("Выпуски", ДанныеИзКэша.Выпуски);
		
	Иначе
		
		// === ВАРИАНТ Б: Полный расчёт (медленно, с записью в кэш) ===
	...
	// ОТКЛЮЧЕНО: Пакетная обработка не работает правильно, когда Знаменатель=0.
	// В этом случае типовой запрос использует автоматическую пропорцию по количеству,
	// ПАКЕТНАЯ ОБРАБОТКА v0.1.1.94: JOIN + замена &Числитель/&Знаменатель на ВТПарыУзлов
	// Все части запроса модифицируются для получения пропорций из ВТ
	...
	// Кэширование управляется отчётом (строка 1004), не алгоритмом — иначе двойная запись в одной транзакции
	ПараметрыДереваСебестоимости.Вставить("ИспользоватьКэширование", Ложь);
...
#Область ТекстЗапроса
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ФактическиеДанные.Полуфабрикат ЕСТЬ NULL
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|		ИНАЧЕ ФактическиеДанные.Полуфабрикат.ЕдиницаИзмерения
		|	КОНЕЦ КАК ЕдиницаИзмеренияПолуфабриката,
