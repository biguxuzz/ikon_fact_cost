...
Функция ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева)
	...
	Если НЕ ПараметрыДерева.Свойство("ЖурналВерсииЗаписан") Тогда
		ЗаписьЖурналаРегистрации("ikon_cost_ПараметрыУзлаСКэшем", УровеньЖурналаРегистрации.Информация,,,
			"v0.1.1.286: Исправлено разузлование возвратных отходов - проверка категории по номенклатуре полуфабриката вместо текущей затраты");
...
Процедура ikon_cost_ПостроитьДеревоСебестоимости(ПараметрыДерева, ПараметрыУзла = Неопределено) Экспорт
	...
	Если НЕ ПараметрыДерева.Свойство("ОтходыВозвратныеКатегория") Тогда
		...
		Запрос.Текст =
			...
			|ИЗ
			|	Справочник.ikon_КатегорииПродуктов КАК Категории
			|ГДЕ
			|	Категории.Наименование = &Наименование";
		Запрос.УстановитьПараметр("Наименование", "Отходы возвратные");
		РезультатЗапроса = Запрос.Выполнить();
		Если РезультатЗапроса.Пустой() Тогда
			ПараметрыДерева.Вставить("ОтходыВозвратныеКатегория", Неопределено);
		Иначе
			Выборка = РезультатЗапроса.Выбрать();
			Выборка.Следующий();
			ПараметрыДерева.Вставить("ОтходыВозвратныеКатегория", Выборка.Категория);
		КонецЕсли;
	КонецЕсли;
		...
		Пока УзлыТекущегоУровня.Количество() > 0 Цикл
			...
			Если ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц"
				...
				Если ПорогУровняДляВыгрузки >= 0 Тогда
					...
					Если КоличествоВыгруженных > 0 Тогда
						ЗаписьЖурналаРегистрации("ikon_cost_ВыгрузкаЧанка", УровеньЖурналаРегистрации.Информация,,,
							СтрШаблон("Уровень %1: предварительная выгрузка %2 строк (порог %3)",
								НомерУровня, КоличествоВыгруженных, ПорогЧанка));
					КонецЕсли;
			...
			Пока НачалоПодпакета < УзлыТекущегоУровня.Количество() Цикл
				...
				Если ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц"
					...
					Если КоличествоВыгруженных > 0 Тогда
						ЗаписьЖурналаРегистрации("ikon_cost_ВыгрузкаЧанка", УровеньЖурналаРегистрации.Информация,,,
							СтрШаблон("Уровень %1: выгрузка по порогу %2 строк (порог %3)",
								НомерУровня, КоличествоВыгруженных, ПорогЧанка));
					КонецЕсли;
				...
				Если ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц"
					...
					Если КоличествоВыгруженных > 0 Тогда
						ЗаписьЖурналаРегистрации("ikon_cost_ВыгрузкаЧанка", УровеньЖурналаРегистрации.Информация,,,
							СтрШаблон("Уровень %1: выгрузка после подпакета %2 строк (порог %3)",
								НомерУровня, КоличествоВыгруженных, ПорогЧанка));
					КонецЕсли;
			...
			Если ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц"
				...
				Если ПорогУровняДляВыгрузки >= 0 Тогда
					...
					Если КоличествоВыгруженных > 0 Тогда
						ЗаписьЖурналаРегистрации("ikon_cost_ВыгрузкаЧанка", УровеньЖурналаРегистрации.Информация,,,
							СтрШаблон("Уровень %1: инкрементальная выгрузка %2 строк",
								НомерУровня, КоличествоВыгруженных));
					КонецЕсли;
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры, СуммыЧислителейПоПарам)
	...
	// Диагностика памяти
	ЗаписьЖурналаРегистрации("ikon_cost_Память", УровеньЖурналаРегистрации.Информация,,,
		СтрШаблон("Потоковая обработка завершена. Всего=%1, распределено=%2, нераспределено=%3, таблиц=%4",
			КоличествоВсехСтрок,
			КоличествоРаспределённых,
			КоличествоНераспределённых,
			КоличествоСозданныхТаблиц));
