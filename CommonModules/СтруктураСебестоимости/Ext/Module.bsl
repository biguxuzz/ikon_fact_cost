...
Функция ikon_cost_ПараметрыДереваСебестоимости()
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		"Расширение ikon_cost применено успешно. Версия 0.1.1.156. Алгоритм: " + ?(ПараметрыДерева.ИспользоватьПакетнуюОбработку, "НОВЫЙ", "СТАРЫЙ"));
	//#КонецВставки
...
Функция РазузловатьУзлыПакетом(ПараметрыДерева, УзлыДляОбработки)
	...
	Для Каждого Узел Из УзлыДляОбработкиПослеПроверки Цикл
		...
		Для Инд = 0 По Отборы.ПартииПродукции.ВГраница() Цикл
			...
			// Если Знаменатель = 0, используем вычисленную сумму числителей для пары
			Если ЗнаменательУзла = 0 Тогда
				// Используем единый формат ключа через СоздатьКлючПары
				КлючПары = СоздатьКлючПары(Партия, Аналитика);
				ЗнаменательВычисленный = СуммыЧислителейПоПарам.Получить(КлючПары);
				Если ЗнаменательВычисленный = Неопределено Тогда
					ЗнаменательВычисленный = 0;
				КонецЕсли;
			Иначе
				ЗнаменательВычисленный = ЗнаменательУзла;
			КонецЕсли;
			
			// КЛЮЧ ВКЛЮЧАЕТ Числитель И вычисленный Знаменатель
			КлючПолный = СоздатьКлючПолный(Партия, Аналитика, ЧислительУзла, ЗнаменательВычисленный);
			...
			Если Группа = Неопределено Тогда
				...
				Группа.Вставить("Знаменатель", ЗнаменательВычисленный);
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры, СуммыЧислителейПоПарам)
	...
	// Собираем уникальные пары (без дублирования по Числитель/Знаменатель)
	...
	Для Каждого КлючИЗначение Из ГруппыПоПарам Цикл
		...
		Если УникальныеПары.Получить(КлючПары) = Неопределено Тогда
			...
			// КРИТИЧНО: Устанавливаем Числитель = Знаменатель = 1
			// Это даёт коэффициент = 1 в SQL, и SQL вернёт 100% себестоимости
			// Индивидуальные коэффициенты применятся в пост-обработке
			НоваяСтрока.Числитель = 1;
			НоваяСтрока.Знаменатель = 1;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого КлючИЗначение Из ГруппыПоПарам Цикл
		...
		// Вычисляем коэффициент (ДОЛЮ) для этой группы внутри пары:
		// Коэффициент = |ЧислительГруппы| / СуммаВсехЧислителейПары
		// SQL вернул 100% стоимости для пары, распределяем пропорционально
		СуммаЧислителейПары = СуммыЧислителейПоПарам.Получить(КлючПары);
		ЧислительАбс = Макс(-Группа.Числитель, Группа.Числитель); // |Числитель|
		
		Если СуммаЧислителейПары = Неопределено ИЛИ СуммаЧислителейПары = 0 ИЛИ ЧислительАбс = 0 Тогда
			// Нет данных о сумме или числитель=0 - пропускаем узел (не должно попадать в результат)
			Коэффициент = 0;
		Иначе
