...
Функция ikon_cost_ПараметрыДереваСебестоимости()
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		"Расширение ikon_cost применено успешно. Версия 0.1.1.74 ГИБРИД. Алгоритм: " + ?(ПараметрыДерева.ИспользоватьПакетнуюОбработку, "НОВЫЙ (гибридный: уникальные пакетом, дубликаты индивидуально)", "СТАРЫЙ (рекурсивный)"));
	//#КонецВставки
...
Функция РазузловатьУзлыПакетом(ПараметрыДерева, УзлыДляОбработки)
	...
	//#КонецУдаления
	//#Вставка
	// === ГИБРИДНАЯ ОБРАБОТКА: УНИКАЛЬНЫЕ ПАКЕТОМ, ДУБЛИКАТЫ ИНДИВИДУАЛЬНО ===
	// Это решает проблему декартова произведения при одинаковых парах (Партия, Аналитика)
	...
	// === РАЗДЕЛЕНИЕ УЗЛОВ НА УНИКАЛЬНЫЕ И ДУБЛИКАТЫ ===
	// Считаем сколько раз встречается каждая пара (Партия, Аналитика)
	СчётчикПар = Новый Соответствие;
	Для Каждого Узел Из УзлыДляОбработкиПослеПроверки Цикл
		...
		Для Инд = 0 По Отборы.ПартииПродукции.ВГраница() Цикл
			...
			Счёт = СчётчикПар.Получить(КлючПары);
			Если Счёт = Неопределено Тогда
				Счёт = 0;
			КонецЕсли;
			СчётчикПар.Вставить(КлючПары, Счёт + 1);
		КонецЦикла;
	КонецЦикла;
	
	// Разделяем узлы на две группы
	УзлыДляПакета = Новый Массив; // Узлы с уникальными парами
	УзлыДляИндивидуальнойОбработки = Новый Массив; // Узлы с дублирующимися парами
	...
	Для Каждого Узел Из УзлыДляОбработкиПослеПроверки Цикл
		...
		Для Инд = 0 По Отборы.ПартииПродукции.ВГраница() Цикл
			...
			КонецЕсли;
			КлючПары = СоздатьКлючПары(Партия, Аналитика);
			Если СчётчикПар.Получить(КлючПары) > 1 Тогда
		...
		Если ЕстьДубликаты Тогда
			УзлыДляИндивидуальнойОбработки.Добавить(Узел);
		Иначе
			УзлыДляПакета.Добавить(Узел);
		КонецЕсли;
	КонецЦикла;
	
	// ДИАГНОСТИКА: сколько узлов в каждой группе
	ЗаписьЖурналаРегистрации("СтруктураСебестоимости.Диагностика",
		УровеньЖурналаРегистрации.Предупреждение,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ГИБРИД: всего=%1, пакетом=%2, индивидуально=%3",
			Строка(УзлыДляОбработкиПослеПроверки.Количество()),
			Строка(УзлыДляПакета.Количество()),
			Строка(УзлыДляИндивидуальнойОбработки.Количество())));
	
	// === ОБРАБОТКА УЗЛОВ С УНИКАЛЬНЫМИ ПАРАМИ (ПАКЕТОМ) ===
	Если УзлыДляПакета.Количество() > 0 Тогда
		ПервыйУзел = УзлыДляПакета[0];
		...
		МассивПараметровУзлов = Новый Массив;
		СоответствиеУзлов = Новый Соответствие;
		Для Каждого Узел Из УзлыДляПакета Цикл
			МассивПараметровУзлов.Добавить(Узел.ПараметрыУзла);
			КлючУзла = СоздатьКлючУзла(Узел.ПараметрыУзла);
			СоответствиеУзлов.Вставить(КлючУзла, Узел);
		КонецЦикла;
		
		ВремяДоЗапроса = ТекущаяДатаСеанса();
		РезультатПоУзлам = СебестоимостьУзловПакетомJOIN(МассивПараметровУзлов, ВыводитьДопРасходы, ПараметрыДерева);
	...
	// === ОБРАБОТКА УЗЛОВ С ДУБЛИРУЮЩИМИСЯ ПАРАМИ (ИНДИВИДУАЛЬНО, КАК В СТАРОМ АЛГОРИТМЕ) ===
	Для Каждого Узел Из УзлыДляИндивидуальнойОбработки Цикл
		УровеньУзла = Узел.ОписаниеПродукции.Уровень + 1;
		ВыводитьДопРасходы = Узел.ПараметрыУзла.Отборы.РазворачиватьДопРасходы И НЕ (УровеньУзла = 1);
