...
Функция ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева)
	...
	// Версия алгоритма: v1.0.0.316 - восстановлены ДатаНачала и ВалютаУправленческогоУчета в фоновом модуле
	...
	Для Каждого ТекПартия Из СебестоимостьУзла Цикл
		...
		Если ПараметрыДерева.ТипРезультата = "ДеревоЗначений"
			...
			Если ТекПартия.ТребуетсяРазузлование
				...
				// v1.0.0.312: Для МВТ обнуляем постатейные на полуфабрикатах
				// Аналогично CASE в отчёте: WHEN ТребуетсяРазузлование THEN 0
				Если ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц" Тогда
					СтрокаПартия.ПостатейныеПостоянные = 0;
					СтрокаПартия.ПостатейныеПеременные = 0;
				КонецЕсли;
...
//
// Изменения относительно базового модуля СтруктураСебестоимости:
//  - v1.0.0.310: Строка ПРОДУКЦИИ (уровень 0) НЕ создаётся для МВТ+разузлование (аналогично типовому алгоритму).
//    ДобавлятьПолуфабрикатыВРезультат не влияет на уровень 0 — только на полуфабрикаты (уровень 1+).
...
Процедура ikon_cost_ЗаполнитьДеревоСебестоимости(ПараметрыДерева, ПараметрыУзла)
	...
	Для Каждого ТекПартия Из ВыпущеннаяПродукция Цикл
		...
		// v1.0.0.311: Строка продукции (уровень 0) создаётся всегда для МВТ
		// (нужна отчёту для JOIN по ПартияПродукции)
		// Но при МВТ + ТребуетсяРазузлование постатейные обнуляются (см. ниже после заполнения)
		Если Истина Тогда
			СтрокаПартия = Результат.Добавить(); // СтрокаДереваЗначений
			ЗаполнитьЗначенияСвойств(СтрокаПартия, ТекПартия);
			...
			// v1.0.0.311: Для МВТ с разузлованием обнуляем постатейные на корневой строке
			// Отчёт обнуляет их через CASE (ТребуетсяРазузлование → 0),
			// но в кэше эти значения должны быть = 0, иначе при распределении
			// по коэффициентам на уровнях разузлования произойдёт завышение
			Если ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц"
				И ТекПартия.ТребуетсяРазузлование Тогда
				СтрокаПартия.ПостатейныеПостоянные = 0;
				СтрокаПартия.ПостатейныеПеременные = 0;
			КонецЕсли;
		КонецЕсли;
		...
		ПараметрыУзлаПартии = ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева);
			...
			Если ПараметрыДерева.Свойство("ИспользоватьПакетнуюОбработку") 
				...
				// v1.0.0.310: СтрокаПартия может быть не создана для МВТ+разузлование
				// v1.0.0.311: СтрокаПартия всегда создаётся (Если Истина на уровне 0)
				УсловиеСозданияСтроки = Истина;
				Если УсловиеСозданияСтроки Тогда
					Узел.Вставить("СтрокаПартия", СтрокаПартия);
				Иначе
					Узел.Вставить("СтрокаПартия", Неопределено);
				КонецЕсли;
...
Функция РазузловатьУзлыПакетом(ПараметрыДерева, УзлыДляОбработки)
	...
	// v1.0.0.306: Уникальный индекс в КлючУзла для предотвращения коллизий
	// при нескольких партиях корневого продукта, потребляющих одну партию полуфабриката
	ИндексУзла = 0;
	Для Каждого Узел Из УзлыДляОбработкиПослеПроверки Цикл
		Отборы = Узел.ПараметрыУзла.Отборы;
		КлючУзла = СоздатьКлючУзла(Узел.ПараметрыУзла) + "|" + ИндексУзла;
		Узел.Вставить("УникальныйКлюч", КлючУзла);
	...
	Для Каждого Узел Из УзлыДляОбработкиПослеПроверки Цикл
		Отборы = Узел.ПараметрыУзла.Отборы;
		// v1.0.0.306: Используем уникальный ключ из ПРОХОДА 1 (включает индекс)
		КлючУзла = Узел.УникальныйКлюч;
