...
Функция ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева)
	...
	Если НЕ ПараметрыДерева.Свойство("ЖурналВерсииЗаписан") Тогда
		ЗаписьЖурналаРегистрации("ikon_cost_ПараметрыУзлаСКэшем", УровеньЖурналаРегистрации.Информация,,,
			"v0.1.1.247: Исправление дублирующихся записей в кэш - явная дедупликация по строковому ключу перед записью в регистр (пустые ссылки составного типа).");
...
Процедура ikon_cost_ЗаполнитьДеревоСебестоимости(ПараметрыДерева, ПараметрыУзла)
	...
	// === СОХРАНЕНИЕ В КЭШ (после успешного расчёта) ===
	Если ПараметрыДерева.Свойство("ИспользоватьКэширование")
		И ПараметрыДерева.ИспользоватьКэширование
		И ПараметрыДерева.ТипРезультата = "МенеджерВременныхТаблиц"
		И ПараметрыДерева.Свойство("ПериодРасчёта") Тогда

		// Сохраняем результат в кэш
		Попытка
			// Проверяем существование свойства ДанныеПоСебестоимости (исправление v0.1.1.232)
			ДанныеПоСебестоимости = 0; // Значение по умолчанию
			Если ПараметрыУзла.Свойство("Отборы")
				И ПараметрыУзла.Отборы.Свойство("ДанныеПоСебестоимости") Тогда
				ДанныеПоСебестоимости = ПараметрыУзла.Отборы.ДанныеПоСебестоимости;
			КонецЕсли;

			ikon_cost_КэшированиеРасчётов.ЗаписатьРезультатВКэш(
				ПараметрыДерева.Результат, // МенеджерВТ с таблицей Результат
				ПараметрыДерева.ПериодРасчёта,
				ПараметрыДерева.ВерсияАлгоритма,
				ДанныеПоСебестоимости,
				ПараметрыДерева.ДатаОкончания);

			ЗаписьЖурналаРегистрации("ikon_cost_Кэширование", УровеньЖурналаРегистрации.Информация, , ,
				"Сохранено в кэш: период=" + Формат(ПараметрыДерева.ПериодРасчёта, "ДФ=yyyy-MM-dd") +
				", версия=" + ПараметрыДерева.ВерсияАлгоритма);
		Исключение
			ЗаписьЖурналаРегистрации("ikon_cost_Кэширование", УровеньЖурналаРегистрации.Ошибка, , ,
				"Ошибка сохранения в кэш: " + ОписаниеОшибки());
		КонецПопытки;
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры, СуммыЧислителейПоПарам)
	...
	// ИСПРАВЛЕНИЕ v0.1.1.242: Проверяем наличие обязательных полей ТребуетсяРазузлование и ТребуетсяРазузлованиеДопРасходов
	// Если поля отсутствуют в результате запроса, добавляем их с значением Ложь
	Если ВсеДанные.Колонки.Найти("ТребуетсяРазузлование") = Неопределено Тогда
		ВсеДанные.Колонки.Добавить("ТребуетсяРазузлование", Новый ОписаниеТипов("Булево"));
		Для Каждого Строка Из ВсеДанные Цикл
			Строка.ТребуетсяРазузлование = Ложь;
		КонецЦикла;
	КонецЕсли;
	Если ВсеДанные.Колонки.Найти("ТребуетсяРазузлованиеДопРасходов") = Неопределено Тогда
		ВсеДанные.Колонки.Добавить("ТребуетсяРазузлованиеДопРасходов", Новый ОписаниеТипов("Булево"));
		Для Каждого Строка Из ВсеДанные Цикл
			Строка.ТребуетсяРазузлованиеДопРасходов = Ложь;
		КонецЦикла;
	КонецЕсли;
	// Проверяем наличие поля НазначениеЗатрата (может отсутствовать для некоторых типов затрат)
	Если ВсеДанные.Колонки.Найти("НазначениеЗатрата") = Неопределено Тогда
		ВсеДанные.Колонки.Добавить("НазначениеЗатрата");
	КонецЕсли;
	...
	// === АГРЕГАЦИЯ ДУБЛИКАТОВ В КАЖДОЙ ТАБЛИЦЕ УЗЛА ===
	// Проблема: могут возникнуть дубликаты записей "Постатейные" с одинаковыми ключами
	// (включая NULL-значения для необязательных измерений), что приводит к ошибке записи в кэш.
	// Решение: агрегируем строки с одинаковыми ключами путём сложения всех числовых полей.
	Для Каждого КлючИЗначение Из РезультатПоУзлам Цикл
		ТаблицаУзла = КлючИЗначение.Значение;
		Если ТаблицаУзла = Неопределено ИЛИ ТаблицаУзла.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;

		// Проверяем, есть ли дубликаты перед свёрткой
		// Дубликаты - это строки с одинаковыми значениями ключевых полей измерений
		ЕстьДубликаты = Ложь;

		// Формируем ключ для каждой строки и проверяем дубликаты
		// ИСПРАВЛЕНО v0.1.1.238: Исправлены имена полей в соответствии с результатами запроса
		// Уровень - не возвращается запросом, удалён из ключа
		// Номенклатура → Затрата
		// Характеристика → ХарактеристикаЗатрата
		// Серия → СерияЗатрата
		СписокКлючей = Новый Массив;
		Для Каждого Строка Из ТаблицаУзла Цикл
			КлючСтроки = Строка(Строка.ПартияВыходноеИзделие) + "|"
				+ Строка(Строка.АналитикаУчетаПартийВыходноеИзделие) + "|"
				+ Строка(Строка.Затрата) + "|"
				+ Строка(Строка.ХарактеристикаЗатрата) + "|"
				+ Строка(Строка.СерияЗатрата) + "|"
				+ Строка(Строка.ПартияЗатрата) + "|"
				+ Строка(Строка.АналитикаУчетаПартийЗатрата) + "|"
				+ Строка(Строка.ТипЗатрат) + "|"
				+ Строка(Строка.Подразделение);

			Если СписокКлючей.Найти(КлючСтроки) <> Неопределено Тогда
				ЕстьДубликаты = Истина;
				Прервать;
			КонецЕсли;
			СписокКлючей.Добавить(КлючСтроки);
		КонецЦикла;

		// Если есть дубликаты, выполняем свёртку
		Если ЕстьДубликаты Тогда
			// Свертываем таблицу по ключевым полям измерений, суммируя ресурсы
			// ИСПРАВЛЕНО v0.1.1.239: Исправлены имена полей в соответствии с результатами запроса
			// ИСПРАВЛЕНИЕ v0.1.1.243: Добавлены поля ТребуетсяРазузлование и ТребуетсяРазузлованиеДопРасходов в ключевые поля
			// чтобы они не удалялись при свёртке (операция Свернуть() удаляет поля не указанные в ключах или ресурсах)
			// ИСПРАВЛЕНИЕ v0.1.1.245: Добавлены недостающие поля в КлючевыеПоля для Свернуть().
		// Свернуть() удаляет все колонки, не указанные ни в ключевых, ни в ресурсных полях.
		// Эти поля нужны при обходе ТекПартия в ОбработатьРезультатыУзла (строки 1268-1331).
		КлючевыеПоля = "ПартияВыходноеИзделие,АналитикаУчетаПартийВыходноеИзделие,Затрата,ХарактеристикаЗатрата,СерияЗатрата,НазначениеЗатрата,ЕдиницаИзмеренияЗатрата,АналитикаУчетаЗатрата,СтатьяКалькуляции,ВключитьНДСВСтоимость,ИсключитьНДСИзСтоимости,ПартияЗатрата,АналитикаУчетаПартийЗатрата,ТипЗатрат,Подразделение,ТребуетсяРазузлование,ТребуетсяРазузлованиеДопРасходов";
			РесурсныеПоля = "КоличествоЗатрата,СуммаЗатрата,СуммаЗатратаЗабалансовая,Материальные,Трудозатраты,ПостатейныеПостоянные,ПостатейныеПеременные,ДопРасходы,НДД,МатериальныеСНДС,ТрудозатратыСНДС,ПостатейныеПостоянныеСНДС,ПостатейныеПеременныеСНДС,ДопРасходыСНДС,МатериальныеБезНДС,ПостатейныеПостоянныеБезНДС,ПостатейныеПеременныеБезНДС,ДопРасходыБезНДС,МатериальныеУпр,ТрудозатратыУпр,ПостатейныеПостоянныеУпр,ПостатейныеПеременныеУпр,ДопРасходыУпр,СтоимостьЗабалансоваяУпр,МатериальныеРегл,ТрудозатратыРегл,ПостатейныеПостоянныеРегл,ПостатейныеПеременныеРегл,ДопРасходыРегл,СтоимостьЗабалансоваяРегл,НалоговыйУчет,ПостояннаяРазница,ВременнаяРазница";
			ТаблицаУзла.Свернуть(КлючевыеПоля, РесурсныеПоля);
		КонецЕсли;
	КонецЦикла;
