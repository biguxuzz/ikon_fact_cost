...
Функция ikon_cost_ПараметрыДереваСебестоимости()
	...
	ЗаписьЖурналаРегистрации("ikon_cost_ПараметрыДереваСебестоимости", УровеньЖурналаРегистрации.Информация,,,
		"v0.1.1-219: Кеширование СхемаКД в ПараметрыДерева, отказ от НастройкиПоУмолчанию на каждый узел для снижения памяти");
	...
	ПараметрыДерева.Вставить("КэшСхемаКД", Неопределено);
...
Функция ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева)
	...
	Если ПараметрыДерева.КэшСхемаКД = Неопределено Тогда
		ПараметрыДерева.КэшСхемаКД = Отчеты.ДеревоСебестоимостиПродукции.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
	КонецЕсли;
...
Функция СебестоимостьУзловПакетомJOIN(МассивПараметровУзлов, ВыводитьДопРасходы, ОбщиеПараметры)
	...
	// 3. Получаем базовый запрос и модифицируем его для формирования ВТ на стороне СУБД
	Запрос = ЗапросВыпущеннаяПродукцияПакетный(ПараметрыУзлаОбъединенные, Истина, ТаблицаПарУзлов);
	Запрос.Текст = Запрос.Текст + ТекстЗапросаЗатратыНаПродукцию(ВыводитьДопРасходы);
	
	// 3.1 Модифицируем финальный SELECT для формирования ВТДанныеБезКлюча на стороне СУБД
	// Находим последний SELECT после последнего ";" в тексте запроса
	ТекстЗапроса = Запрос.Текст;
	ТекстЗапросаИсходный = ТекстЗапроса;
	
	// Проверяем, нет ли уже ПОМЕСТИТЬ в финальном запросе
	ПозицияПоследнейТочкиСЗапятой = СтрНайти(ТекстЗапроса, ";", НаправлениеПоиска.СКонца);
	ФинальныйБлок = "";
	Если ПозицияПоследнейТочкиСЗапятой > 0 Тогда
		ФинальныйБлок = Сред(ТекстЗапроса, ПозицияПоследнейТочкиСЗапятой + 1);
	Иначе
		ФинальныйБлок = ТекстЗапроса;
	КонецЕсли;
	
	// Защита: если в финальном блоке уже есть ПОМЕСТИТЬ, не вставляем повторно
	Если СтрНайти(ВРег(ФинальныйБлок), "ПОМЕСТИТЬ") = 0 Тогда
		// Ищем первое вхождение ИЗ (с учётом |ИЗ или \nИЗ)
		ПозицияИзСтрокой = СтрНайти(ФинальныйБлок, Символы.ПС + "|ИЗ");
		Если ПозицияИзСтрокой = 0 Тогда
			ПозицияИзСтрокой = СтрНайти(ФинальныйБлок, "|ИЗ");
		КонецЕсли;
		
		Если ПозицияИзСтрокой > 0 Тогда
			// Вставляем ПОМЕСТИТЬ перед первым ИЗ
			ЧастьДоИз = Лев(ФинальныйБлок, ПозицияИзСтрокой - 1);
			ЧастьОтИз = Сред(ФинальныйБлок, ПозицияИзСтрокой);
			ФинальныйБлок = ЧастьДоИз + Символы.ПС + "|ПОМЕСТИТЬ ВТДанныеБезКлюча" + ЧастьОтИз;
			
			// Собираем обратно весь текст запроса
			Если ПозицияПоследнейТочкиСЗапятой > 0 Тогда
				Запрос.Текст = Лев(ТекстЗапроса, ПозицияПоследнейТочкиСЗапятой) + ФинальныйБлок;
			Иначе
				Запрос.Текст = ФинальныйБлок;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	// Выполняем запрос без выгрузки - данные остаются на стороне СУБД во ВТДанныеБезКлюча
	Запрос.Выполнить();
	
	// 3.2 Получаем структуру колонок без данных
	ЗапросСтруктуры = Новый Запрос;
	ЗапросСтруктуры.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
	
	// Берём структуру колонок через тот же запрос, но с ПЕРВЫЕ 0 (чтобы не тянуть данные)
	// Делается через исходный текст, чтобы не зависеть от синтаксиса ВЫБРАТЬ * (не всегда доступен)
	ПозицияПоследнейТочкиСЗапятойСтруктуры = СтрНайти(ТекстЗапросаИсходный, ";", НаправлениеПоиска.СКонца);
	ФинальныйБлокСтруктуры = "";
	Если ПозицияПоследнейТочкиСЗапятойСтруктуры > 0 Тогда
		ФинальныйБлокСтруктуры = Сред(ТекстЗапросаИсходный, ПозицияПоследнейТочкиСЗапятойСтруктуры + 1);
	Иначе
		ФинальныйБлокСтруктуры = ТекстЗапросаИсходный;
	КонецЕсли;
	
	Если СтрНайти(ВРег(ФинальныйБлокСтруктуры), "ПЕРВЫЕ") = 0 Тогда
		КлючСРазрешенными = "ВЫБРАТЬ РАЗРЕШЕННЫЕ";
		КлючВыбрать = "ВЫБРАТЬ";
		ПозицияКлюча = СтрНайти(ВРег(ФинальныйБлокСтруктуры), КлючСРазрешенными);
		ДлинаКлюча = 0;
		Если ПозицияКлюча > 0 Тогда
			ДлинаКлюча = СтрДлина(КлючСРазрешенными);
		Иначе
			ПозицияКлюча = СтрНайти(ВРег(ФинальныйБлокСтруктуры), КлючВыбрать);
			ДлинаКлюча = СтрДлина(КлючВыбрать);
		КонецЕсли;
		
		Если ПозицияКлюча > 0 Тогда
			ПозицияВставки = ПозицияКлюча + ДлинаКлюча - 1;
			ФинальныйБлокСтруктуры =
				Лев(ФинальныйБлокСтруктуры, ПозицияВставки)
				+ " ПЕРВЫЕ 0"
				+ Сред(ФинальныйБлокСтруктуры, ПозицияВставки + 1);
		КонецЕсли;
	КонецЕсли;
	
	Если ПозицияПоследнейТочкиСЗапятойСтруктуры > 0 Тогда
		ЗапросСтруктуры.Текст = Лев(ТекстЗапросаИсходный, ПозицияПоследнейТочкиСЗапятойСтруктуры) + ФинальныйБлокСтруктуры;
	Иначе
		ЗапросСтруктуры.Текст = ФинальныйБлокСтруктуры;
	КонецЕсли;
	
	ПустаяТаблицаСтруктуры = ЗапросСтруктуры.Выполнить().Выгрузить();
	
	// 3.3 Строим список колонок на основе структуры
	МассивКолонок = Новый Массив;
	Для Каждого Колонка Из ПустаяТаблицаСтруктуры.Колонки Цикл
	...
	// 4.1 Выполняем JOIN для добавления КлючУзла
	...
	// Инициализируем пустые таблицы (без колонки КлючУзла) - используем структуру из СУБД
	Для Каждого ПараметрыУзла Из МассивПараметровУзлов Цикл
		КлючУзла = СоздатьКлючУзла(ПараметрыУзла);
		ТаблицаУзла = ПустаяТаблицаСтруктуры.СкопироватьКолонки();
		РезультатПоУзлам.Вставить(КлючУзла, ТаблицаУзла);
	КонецЦикла;
