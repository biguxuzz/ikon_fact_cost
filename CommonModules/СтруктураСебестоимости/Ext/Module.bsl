...
Функция ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева)
	...
	Если НЕ ПараметрыДерева.Свойство("ЖурналВерсииЗаписан") Тогда
		ЗаписьЖурналаРегистрации("ikon_cost_ПараметрыУзлаСКэшем", УровеньЖурналаРегистрации.Информация,,,
			"v0.1.1.280: Оптимизация потребления памяти - потоковая обработка нераспределённых строк, индекс групп, ленивое создание таблиц");
...
Процедура ikon_cost_ПостроитьДеревоСебестоимости(ПараметрыДерева, ПараметрыУзла = Неопределено) Экспорт
	...
	Если ПараметрыДерева.Свойство("ИспользоватьПакетнуюОбработку")
		...
		ПараметрыДерева.Вставить("РазмерПодпакетаУзлов", 500);
	КонецЕсли;
	...
	Если НЕ ПараметрыДерева.Свойство("МаксимальныйРазмерПодпакета") Тогда
		...
		ПараметрыДерева.Вставить("МаксимальныйРазмерПодпакета", 500);
	КонецЕсли;
	...
	Если НЕ ПараметрыДерева.Свойство("ПорогВыгрузкиВМВТ") Тогда
		...
		ПараметрыДерева.Вставить("ПорогВыгрузкиВМВТ", 5000); // Строк в памяти перед принудительной выгрузкой
	КонецЕсли;
	...
	Если НЕ ПараметрыДерева.Свойство("МаксимальныйРазмерЧанкаВыгрузки") Тогда
		ПараметрыДерева.Вставить("МаксимальныйРазмерЧанкаВыгрузки", 5000);
	КонецЕсли;
	Если НЕ ПараметрыДерева.Свойство("МинимальныйРазмерЧанкаВыгрузки") Тогда
		ПараметрыДерева.Вставить("МинимальныйРазмерЧанкаВыгрузки", 500);
	КонецЕсли;
...
Процедура ikon_cost_РазузловатьУзелДереваСебестоимости(ПараметрыДерева, ПараметрыУзла, ТаблицаРезультата, ОписаниеПродукции)
	...
	Для Каждого ТекПартия Из СебестоимостьУзла Цикл
		...
		// ПРОВЕРКА НА ОТХОДЫ ВОЗВРАТНЫЕ
		// Если категория продукта = "Отходы возвратные", разузлование прекращается
		// Отходы возвратные не должны включать затраты предыдущих уровней в свою себестоимость
		ЭтоОтходыВозвратные = Ложь;
		Если ПараметрыДерева.Свойство("ОтходыВозвратныеКатегория")
			И ПараметрыДерева.ОтходыВозвратныеКатегория <> Неопределено Тогда
			// Безопасная проверка наличия реквизита перед использованием
			КатегорияПродукта = Неопределено;
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта(
					"ikon_КатегорияПродукта", 
					ТекПартия.Затрата.Метаданные()) Тогда
				КатегорияПродукта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ТекПартия.Затрата, 
					"ikon_КатегорияПродукта");
			КонецЕсли;
			ЭтоОтходыВозвратные = (ПараметрыДерева.ОтходыВозвратныеКатегория = КатегорияПродукта);
		КонецЕсли;
		// КОНЕЦ ПРОВЕРКИ НА ОТХОДЫ

		Если ТекПартия.ТребуетсяРазузлование И ТекПартия.КоличествоЗатрата <> 0 И НЕ ЭтоОтходыВозвратные Тогда
		...
		Если ПараметрыДерева.ТипРезультата = "ДеревоЗначений"
			...
			Если ТекПартия.ТребуетсяРазузлование
				...
				И НЕ ЭтоОтходыВозвратные Тогда
...
Функция ВычислитьПорогЧанкаПоУровню(ПараметрыДерева, НомерУровня)
	...
	БазовыйРазмер = 1000;  // Уменьшено с 5000 для PostgreSQL
	ПараметрыДерева.Свойство("МаксимальныйРазмерЧанкаВыгрузки", БазовыйРазмер);

	МинимальныйРазмер = 100;  // Уменьшено с 500 для PostgreSQL
