...
Функция ikon_cost_ПараметрыДереваСебестоимости()
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		"Расширение ikon_cost применено успешно. Версия 0.1.1.190. Алгоритм: " + ?(ПараметрыДерева.ИспользоватьПакетнуюОбработку, "НОВЫЙ", "СТАРЫЙ"));
	//#КонецВставки
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры, СуммыЧислителейПоПарам)
	...
	// КРИТИЧНО: Устанавливаем КоличествоПродукции = 1 для доп. расходов!
	// ТекстДопРасходыОбъединение использует * &КоличествоПродукции напрямую.
	// При КоличествоПродукции = 1 SQL вернёт 100% доп. расходов,
	// а пост-обработка применит индивидуальный коэффициент.
	...
	Для Каждого КлючИЗначение Из ГруппыПоПарам Цикл
		...
		Для Каждого КлючУзла Из Группа.КлючиУзлов Цикл
			...
			Для Каждого СтрокаДанных Из СтрокиПары Цикл
				...
				// ТРАССИРОВКА: Антиокислитель 6PPD после применения коэффициента
...
|		ПО ВыпущеннаяПродукция.ПартияПродукции = ВТПарыУзлов.ПартияПродукции
|			И ВыпущеннаяПродукция.АналитикаУчетаПартийПродукции = ВТПарыУзлов.АналитикаУчетаПартийПродукции
|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваров
