...
#Вставка
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		"Расширение ikon_cost применено успешно. Версия 0.1.0.57. Алгоритм: " + ?(ПараметрыДерева.ИспользоватьПакетнуюОбработку, "НОВЫЙ (итерация по уровням)", "СТАРЫЙ (рекурсивный)"));
#КонецВставки
...
Функция РазузловатьУзлыПакетом(ПараметрыДерева, УзлыДляОбработки)
	...
	#КонецУдаления
	#Вставка
	// === ОБРАБОТКА ПО УРОВНЯМ (каждый узел - отдельным запросом) ===
	// Пакетная обработка с JOIN невозможна из-за индивидуальных Числитель/Знаменатель
	// для каждого узла (коэффициенты распределения затрат)
...
Функция СебестоимостьУзловПакетомJOIN(МассивПараметровУзлов, ВыводитьДопРасходы, ОбщиеПараметры)
	...
	Для Каждого ПараметрыУзла Из МассивПараметровУзлов Цикл
		...
		Для Инд = 0 По Отборы.ПартииПродукции.ВГраница() Цикл
			...
			// Индекс: КлючУзла -> КлючПары (для распределения результатов)
			// Каждый узел имеет свою уникальную комбинацию коэффициентов
			ИндексПарПоУзлам.Вставить(КлючУзла, СоздатьКлючПары(Партия, Аналитика));
		КонецЦикла;
	...
	// ДИАГНОСТИКА
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"СебестоимостьУзловПакетомJOIN: узлов=%1, пар=%2, получено строк=%3",
	...
	// 4. Распределяем результаты по узлам
	РезультатПоУзлам = Новый Соответствие;
	...
	// Проверяем наличие колонок для распределения
	Если ВсеДанные.Колонки.Найти("ПартияВыходноеИзделие") = Неопределено
		ИЛИ ВсеДанные.Колонки.Найти("АналитикаУчетаПартийВыходноеИзделие") = Неопределено Тогда
		Возврат РезультатПоУзлам;
	КонецЕсли;
	
	// Распределяем строки по узлам
	Для Каждого СтрокаДанных Из ВсеДанные Цикл
		КлючПары = СоздатьКлючПары(СтрокаДанных.ПартияВыходноеИзделие, 
			СтрокаДанных.АналитикаУчетаПартийВыходноеИзделие);
