...
Функция ikon_cost_ПараметрыДереваСебестоимости()
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		"Расширение ikon_cost применено успешно. Версия 0.1.1.88 СТАРЫЙ-АЛГОРИТМ. Алгоритм: " + ?(ПараметрыДерева.ИспользоватьПакетнуюОбработку, "НОВЫЙ", "СТАРЫЙ"));
	//#КонецВставки
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры)
	...
	// 6. ПОСТ-РАСПРЕДЕЛЕНИЕ С КОЭФФИЦИЕНТАМИ
	// Запрос вернул 100% себестоимости (с &Числитель=0, &Знаменатель=0)
	// Применяем коэффициент Числитель/Знаменатель для каждой группы
	РезультатПоУзлам = Новый Соответствие;
	...
	// Индексируем строки по КлючПары для быстрого поиска
	...
	// Распределяем строки по узлам с применением коэффициентов
	Для Каждого КлючИЗначение Из ГруппыПоПарам Цикл
		Группа = КлючИЗначение.Значение;
		...
		Если Группа.Знаменатель = 0 Тогда
			Коэффициент = 1;
		Иначе
			Коэффициент = Группа.Числитель / Группа.Знаменатель;
		КонецЕсли;
		
		// ДИАГНОСТИКА: Выводим информацию о применяемых коэффициентах (всегда для анализа)
...
Функция ЗапросВыпущеннаяПродукцияГруппировка(ПараметрыУзла, ПомещатьВВТ, ТаблицаГруппПар)
	...
	// КРИТИЧНО: Добавляем JOIN с ВТПарыУзлов для получения Числитель и Знаменатель из группы
	// Ищем место для вставки JOIN (перед ГДЕ)
	ТекстJoin = 
	"	ЛЕВОЕ СОЕДИНЕНИЕ ВТПарыУзлов КАК ВТПарыУзлов
	|		ПО СебестоимостьТоваров.Партия = ВТПарыУзлов.ПартияПродукции
	|			И СебестоимостьТоваров.АналитикаУчетаПартий = ВТПарыУзлов.АналитикаУчетаПартийПродукции
	|";
	
	// Вставляем JOIN перед ГДЕ
	ПозицияГде = СтрНайти(ТекстЗапроса, "ГДЕ");
	Если ПозицияГде > 0 Тогда
		ТекстЗапроса = Лев(ТекстЗапроса, ПозицияГде - 1) + ТекстJoin + Сред(ТекстЗапроса, ПозицияГде);
	КонецЕсли;
	
	// Заменяем выражения с &Числитель и &Знаменатель на поля из ВТПарыУзлов
	// ВАЖНО: Текст запроса СКД НЕ содержит символов "|", поэтому шаблоны должны быть в чистом виде
	// Оригинал: ВЫБОР КОГДА &Числитель = 0 ТОГДА СУММА(...) ИНАЧЕ &Числитель КОНЕЦ КАК Числитель
	// Замена: МАКСИМУМ(ВТПарыУзлов.Числитель) КАК Числитель
	
	// Сначала проверяем, есть ли параметры &Числитель и &Знаменатель в запросе
	...
	ЗаписьЖурналаРегистрации("СтруктураСебестоимости.Диагностика",
		УровеньЖурналаРегистрации.Предупреждение,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ЗапросВыпущеннаяПродукцияГруппировка: &Числитель=%1, &Знаменатель=%2",
			?(ЕстьЧислитель, "ЕСТЬ", "НЕТ"),
			?(ЕстьЗнаменатель, "ЕСТЬ", "НЕТ")));
	
	// Если параметры есть - заменяем через регулярные выражения или упрощённый поиск
	// Упрощённый подход: НЕ заменяем текст запроса, а полагаемся на пост-распределение
	// Запрос вернёт 100% себестоимости (Числитель=0, Знаменатель=0)
	// Пост-распределение применит индивидуальные пропорции каждого узла: Числитель / Знаменатель
...
Функция ikon_cost_ТекстЗапросаЗатратыНаПродукцию(ВыводитьДопРасходы)
	...
	//#КонецУдаления
	//#Вставка
	// ПАКЕТНАЯ ОПТИМИЗАЦИЯ: добавляем ПартияПродукции и АналитикаУчетаПартийПродукции
	// для предотвращения декартова произведения при JOIN
	ТекстМатериальныеЗатраты = ТекстМатериальныеЗатраты + "
	|ВЫБРАТЬ
	|	Т.Организация КАК Организация,
	|	МАКСИМУМ(Т.Период) КАК Период,
	...
	//#КонецУдаления
	//#Вставка
	// ПАКЕТНАЯ ОПТИМИЗАЦИЯ: добавляем ПартияПродукции и АналитикаУчетаПартийПродукции в GROUP BY
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(СебестоимостьТоваров.Период, МЕСЯЦ),
	|	СебестоимостьТоваров.Организация,
	|	СебестоимостьТоваров.ВидЗапасов,
	...
	//#КонецУдаления
	//#Вставка
	// ПАКЕТНАЯ ОПТИМИЗАЦИЯ: добавляем ПартияПродукции и АналитикаУчетаПартийПродукции в JOIN и GROUP BY
	|ИЗ
	|	ВыбытиеИзНЗП КАК Выбытие
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИнтервалыРасходаПартий КАК Интервалы
	|		ПО Выбытие.АналитикаУчетаНоменклатуры = Интервалы.АналитикаУчетаНоменклатуры
	...
	//#КонецУдаления
	//#Вставка
	// ПАКЕТНАЯ ОПТИМИЗАЦИЯ: добавляем ПартияПродукции и АналитикаУчетаПартийПродукции в JOIN
	|ИЗ
	|	ПредварительныйРасходПартий КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ РасходПартийСводно КАК Расход
	|		ПО Т.КорАналитикаУчетаНоменклатуры = Расход.КорАналитикаУчетаНоменклатуры
	...
	//#КонецУдаления
	//#Вставка
	// ПАКЕТНАЯ ОПТИМИЗАЦИЯ: КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ - добавляем ПартияПродукции и АналитикаУчетаПартийПродукции
	// для предотвращения декартова произведения при одинаковых КорПартия для разных ПартияПродукции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПервичныеЗатратыПоПериодам КАК ПартииМатериалов
	|		ПО НЗППоПродукции.Период = ПартииМатериалов.Период
	|			И НЗППоПродукции.АналитикаУчетаНоменклатуры = ПартииМатериалов.КорАналитикаУчетаНоменклатуры
	|			И НЗППоПродукции.Партия = ПартииМатериалов.КорПартия
