...
Функция ikon_cost_ПараметрыУзлаСКэшем(ПараметрыДерева)
	...
	Если НЕ ПараметрыДерева.Свойство("ЖурналВерсииЗаписан") Тогда
		ЗаписьЖурналаРегистрации("ikon_cost_ПараметрыУзлаСКэшем", УровеньЖурналаРегистрации.Информация,,,
			"v0.1.1.278: Фильтрация отходов возвратных при разузловании");
...
Процедура ikon_cost_ПостроитьДеревоСебестоимости(ПараметрыДерева, ПараметрыУзла = Неопределено) Экспорт
	...
	Если ПараметрыДерева.Свойство("ИспользоватьПакетнуюОбработку")
		...
		// По умолчанию: обрабатываем по 1000 узлов за раз (будет адаптивно уменьшаться на глубоких уровнях)
		// Можно уменьшить до 100-200 при критичной нехватке памяти, или увеличить до 2000 при избытке.
	...
	Если НЕ ПараметрыДерева.Свойство("МаксимальныйРазмерПодпакета") Тогда
		ПараметрыДерева.Вставить("МаксимальныйРазмерПодпакета", 1000);
	КонецЕсли;
	...
	Если НЕ ПараметрыДерева.Свойство("ПорогВыгрузкиВМВТ") Тогда
		ПараметрыДерева.Вставить("ПорогВыгрузкиВМВТ", 10000); // Строк в памяти перед принудительной выгрузкой
	КонецЕсли;
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры, СуммыЧислителейПоПарам)
	...
	// Инициализируем результат - пустые таблицы для каждого узла
	...
	// Структура для нераспределённых строк (КлючПары -> Массив структур данных)
	НераспределенныеСтрокиПоКлючам = Новый Соответствие;
	...
	// Предварительно собираем РаспределённыеКлючи для быстрой проверки
	...
	Пока Выборка.Следующий() Цикл
		...
		// Проверяем, принадлежит ли строка какой-либо группе
		Если РаспределённыеКлючи.Получить(КлючПары) = Неопределено Тогда
			// Строка не принадлежит ни одной группе - сохраняем для последующей обработки
			СтрокаДанных = Новый Структура;
			Для Каждого Колонка Из РезультатЗапроса.Колонки Цикл
				СтрокаДанных.Вставить(Колонка.Имя, Выборка[Колонка.Имя]);
			КонецЦикла;

			МассивСтрок = НераспределенныеСтрокиПоКлючам.Получить(КлючПары);
			Если МассивСтрок = Неопределено Тогда
				МассивСтрок = Новый Массив;
				НераспределенныеСтрокиПоКлючам.Вставить(КлючПары, МассивСтрок);
			КонецЕсли;
		...
		// Находим все группы с таким ключом пары
		Для Каждого КлючИЗначение Из ГруппыПоПарам Цикл
			Группа = КлючИЗначение.Значение;
			ГруппаКлючПары = СоздатьКлючПары(Группа.Партия, Группа.Аналитика);

			Если ГруппаКлючПары <> КлючПары Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Каждого КлючУзла Из Группа.КлючиУзлов Цикл
				...
				// Инициализируем колонки для каждой таблицы узла отдельно (при первой строке в неё)
				...
				Для Каждого Колонка Из РезультатЗапроса.Колонки Цикл
					...
					// Проверяем, нужно ли умножать на коэффициент
					// ФАЗА 4: O(1) поиск через Соответствие вместо O(N) через Массив.Найти()
	...
	// === ФАЗА 4: ОЧИСТКА ПАМЯТИ ПОСЛЕ ПОТОКОВОЙ ОБРАБОТКИ ===
	// Освобождаем память от больших структур

	// Очищаем нераспределённые строки (если они были обработаны)
	Если НераспределенныеСтрокиПоКлючам.Количество() = 0 Тогда
		НераспределенныеСтрокиПоКлючам = Неопределено;
	КонецЕсли;

	// Удаляем временную структуру РаспределённыеКлючи
	РаспределённыеКлючи = Неопределено;

	// Диагностика памяти
	ЗаписьЖурналаРегистрации("ikon_cost_Память", УровеньЖурналаРегистрации.Информация,,,
		СтрШаблон("Память освобождена. Обработано: всего=%1, распределено=%2, нераспределено=%3",
			КоличествоВсехСтрок,
			КоличествоРаспределённых,
			КоличествоНераспределённых));

	// === ОБРАБОТКА НЕРАСПРЕДЕЛЁННЫХ СТРОК ===
	// Используем существующую логику распределения только по Партии
	Если НераспределенныеСтрокиПоКлючам <> Неопределено И НераспределенныеСтрокиПоКлючам.Количество() > 0 Тогда
		...
		// Распределяем нераспределённые строки
		Для Каждого КлючИЗначение Из НераспределенныеСтрокиПоКлючам Цикл
			...
			Если Запись = Неопределено ИЛИ Запись.КлючиУзлов.Количество() = 0 Тогда
				Продолжить; // Партия не найдена - пропускаем
			КонецЕсли;

			// Распределяем равномерно по узлам партии
			...
			Для Каждого КлючУзла Из Запись.КлючиУзлов Цикл
				...
				Для Каждого СтрокаДанных Из СтрокиПары Цикл
					// Проверяем тип затрат
					ЭтоДопРасходы = (СтрокаДанных.ТипЗатрат = ТипЗатратДопРасходы);
	...
	// Диагностика
	Если КоличествоВсехСтрок > 0 Тогда
		ЗаписьЖурналаРегистрации("ikon_cost_ПотоковаяОбработка", УровеньЖурналаРегистрации.Информация,,,
			СтрШаблон("Обработано: всего=%1, распределено=%2, нераспределено=%3",
				КоличествоВсехСтрок,
				КоличествоРаспределённых,
				КоличествоНераспределённых));
	КонецЕсли;
