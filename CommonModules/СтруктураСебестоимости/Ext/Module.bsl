...
Функция ikon_cost_ПараметрыДереваСебестоимости()
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		"Расширение ikon_cost применено успешно. Версия 0.1.1.215. Алгоритм: " + ?(ПараметрыДерева.ИспользоватьПакетнуюОбработку, "НОВЫЙ", "СТАРЫЙ"));
	//#КонецВставки
...
Процедура ikon_cost_РазузловатьУзелДереваСебестоимости(ПараметрыДерева, ПараметрыУзла, ТаблицаРезультата, ОписаниеПродукции)
	...
	Для Каждого ТекПартия Из СебестоимостьУзла Цикл
		...
		Если ПараметрыДерева.ТипРезультата = "ДеревоЗначений"
			...
			// ТРАССИРОВКА: Антиокислитель 6PPD в СТАРОМ алгоритме
			ИмяЗатраты = Строка(ТекПартия.Затрата);
			Если СтрНайти(ИмяЗатраты, "Антиокислитель 6PPD") > 0 ИЛИ СтрНайти(ИмяЗатраты, "6PPD") > 0 Тогда
				ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.6PPD.СТАРЫЙ",
					УровеньЖурналаРегистрации.Предупреждение,,,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"СТАРЫЙ: Затрата='%1', Кол=%2, Материальные=%3, Уровень=%4",
						ИмяЗатраты,
						Формат(ТекПартия.КоличествоЗатрата, "ЧДЦ=6"),
						Формат(СтрокаПартия.Материальные, "ЧДЦ=2"),
						Строка(УровеньУзла)));
			КонецЕсли;
...
Функция ikon_cost_СебестоимостьУзла(ПараметрыДерева, ПараметрыУзла, ВыводитьДопРасходы)
	...
	// ТРАССИРОВКА: Параметры запроса доп.расходов в непакетном режиме
	Если ВыводитьДопРасходы Тогда
		Отборы = ПараметрыУзла.Отборы;
		ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.ТЗР",
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"[НЕПАКЕТ] ПАРАМЕТРЫ ЗАПРОСА: ПартииПродукции.Количество()=%1, АналитикиУчетаПартийПродукции.Количество()=%2, ПоВсемПартиям=%3, ПоВсемАналитикамПартий=%4, КоличествоПродукции=%5",
				Строка(Отборы.ПартииПродукции.Количество()),
				Строка(Отборы.АналитикиУчетаПартийПродукции.Количество()),
				Строка(Отборы.ПоВсемПартиям),
				Строка(Отборы.ПоВсемАналитикамПартий),
				Строка(Отборы.КоличествоПродукции)));
	КонецЕсли;
	
	Результат = Запрос.Выполнить();
	ТаблицаРезультата = Результат.Выгрузить();
	
	// ТРАССИРОВКА: Результаты запроса доп.расходов в непакетном режиме
	Если ВыводитьДопРасходы Тогда
		Для Каждого СтрокаSQL Из ТаблицаРезультата Цикл
			ИмяЗатраты = Строка(СтрокаSQL.Затрата);
			Если СтрНайти(ИмяЗатраты, "ТЗР") > 0 ИЛИ СтрНайти(ИмяЗатраты, "таможенные пошлины") > 0 Тогда
				ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.ТЗР",
					УровеньЖурналаРегистрации.Предупреждение,,,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"[НЕПАКЕТ] SQL ВЕРНУЛ: Затрата='%1', ДопРасходы=%2, Партия=%3, Аналитика=%4, КоличествоПродукции=%5",
						ИмяЗатраты,
						Формат(СтрокаSQL.ДопРасходы, "ЧДЦ=2"),
						Строка(СтрокаSQL.ПартияВыходноеИзделие),
						Строка(СтрокаSQL.АналитикаУчетаПартийВыходноеИзделие),
						Формат(СтрокаSQL.КоличествоЗатрата, "ЧДЦ=6")));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если ПараметрыДерева.Свойство("УзлыДляПакетнойОбработки") 
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			Если СтрокаРезультата.ТребуетсяРазузлованиеДопРасходов = Истина Тогда
				...
				КоличествоСброшенныхФлагов = КоличествоСброшенныхФлагов + 1;
			КонецЕсли;
		КонецЦикла;
		
		Если КоличествоСброшенныхФлагов > 0 Тогда
			ЗаписьЖурналаРегистрации(
				"СтруктураСебестоимости.СбросФлаговРазузлования",
				УровеньЖурналаРегистрации.Информация,
				,
				,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"Сброшено флагов ТребуетсяРазузлованиеДопРасходов перед финальным сохранением: %1 строк",
					Строка(КоличествоСброшенныхФлагов)));
		КонецЕсли;
...
Функция СебестоимостьУзловПакетомГруппировка(УзлыДляОбработки, ГруппыПоПарам, СоответствиеУзлов, ВыводитьДопРасходы, ОбщиеПараметры, СуммыЧислителейПоПарам)
	...
	// ТРАССИРОВКА: Параметры запроса доп.расходов
	Если ВыводитьДопРасходы Тогда
		Отборы = ПараметрыУзлаОбъединенные.Отборы;
		ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.ТЗР",
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"[ПАКЕТ] ПАРАМЕТРЫ ЗАПРОСА: ПартииПродукции.Количество()=%1, АналитикиУчетаПартийПродукции.Количество()=%2, ПоВсемПартиям=%3, ПоВсемАналитикамПартий=%4, КоличествоПродукции=%5",
				Строка(Отборы.ПартииПродукции.Количество()),
				Строка(Отборы.АналитикиУчетаПартийПродукции.Количество()),
				Строка(Отборы.ПоВсемПартиям),
				Строка(Отборы.ПоВсемАналитикамПартий),
	...
	// ТРАССИРОВКА: ТЗР и другие доп.расходы
	Для Каждого СтрокаSQL Из ВсеДанные Цикл
		ИмяЗатраты = Строка(СтрокаSQL.Затрата);
		// Трассировка ТЗР
		Если СтрНайти(ИмяЗатраты, "ТЗР") > 0 ИЛИ СтрНайти(ИмяЗатраты, "таможенные пошлины") > 0 Тогда
			ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.ТЗР",
				УровеньЖурналаРегистрации.Предупреждение,,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"[ПАКЕТ] SQL ВЕРНУЛ: Затрата='%1', ДопРасходы=%2, Партия=%3, Аналитика=%4, КоличествоПродукции=%5",
					ИмяЗатраты,
					Формат(СтрокаSQL.ДопРасходы, "ЧДЦ=2"),
					Строка(СтрокаSQL.ПартияВыходноеИзделие),
					Строка(СтрокаSQL.АналитикаУчетаПартийВыходноеИзделие),
					Формат(СтрокаSQL.КоличествоЗатрата, "ЧДЦ=6")));
		КонецЕсли;
		// Трассировка Антиокислитель 6PPD
		Если СтрНайти(ИмяЗатраты, "Антиокислитель 6PPD") > 0 ИЛИ СтрНайти(ИмяЗатраты, "6PPD") > 0 Тогда
			ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.6PPD",
				УровеньЖурналаРегистрации.Предупреждение,,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"SQL ВЕРНУЛ: Затрата='%1', Кол=%2, Материальные=%3, Партия=%4",
					ИмяЗатраты,
					Формат(СтрокаSQL.КоличествоЗатрата, "ЧДЦ=6"),
					Формат(СтрокаSQL.Материальные, "ЧДЦ=2"),
					Строка(СтрокаSQL.ПартияВыходноеИзделие)));
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого КлючИЗначение Из ГруппыПоПарам Цикл
		...
		ЗаписьЖурналаРегистрации("СтруктураСебестоимости.КОЭФФ",
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"Группа: Числ=%1, |Числ|=%2, СуммаПары=%3, Коэфф=%4, СтрокПары=%5, Узлов=%6",
				Строка(Группа.Числитель), Строка(ЧислительАбс), Строка(СуммаЧислителейПары),
		...
		Для Каждого КлючУзла Из Группа.КлючиУзлов Цикл
			...
			Для Каждого СтрокаДанных Из СтрокиПары Цикл
				...
				// ТРАССИРОВКА: ТЗР и другие доп.расходы после применения коэффициента
				ИмяЗатраты = Строка(НоваяСтрока.Затрата);
				// Трассировка ТЗР
				Если СтрНайти(ИмяЗатраты, "ТЗР") > 0 ИЛИ СтрНайти(ИмяЗатраты, "таможенные пошлины") > 0 Тогда
					ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.ТЗР",
						УровеньЖурналаРегистрации.Предупреждение,,,
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							"[ПАКЕТ] ПОСЛЕ КОЭФФ: Затрата='%1', ДопРасходы_ДО=%2, ДопРасходы_ПОСЛЕ=%3, Коэфф=%4, КлючУзла=%5, Партия=%6",
							ИмяЗатраты,
							Формат(СтрокаДанных.ДопРасходы, "ЧДЦ=2"),
							Формат(НоваяСтрока.ДопРасходы, "ЧДЦ=2"),
							Формат(Коэффициент, "ЧДЦ=6"),
							КлючУзла,
							Строка(НоваяСтрока.ПартияВыходноеИзделие)));
				КонецЕсли;
				// Трассировка Антиокислитель 6PPD
				Если СтрНайти(ИмяЗатраты, "Антиокислитель 6PPD") > 0 ИЛИ СтрНайти(ИмяЗатраты, "6PPD") > 0 Тогда
					ЗаписьЖурналаРегистрации("СтруктураСебестоимости.ТРАССИРОВКА.6PPD",
						УровеньЖурналаРегистрации.Предупреждение,,,
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							"ПОСЛЕ КОЭФФ: Затрата='%1', Кол_ДО=%2, Кол_ПОСЛЕ=%3, Коэфф=%4, КлючУзла=%5",
							ИмяЗатраты,
							Формат(СтрокаДанных.КоличествоЗатрата, "ЧДЦ=6"),
							Формат(НоваяСтрока.КоличествоЗатрата, "ЧДЦ=6"),
							Формат(Коэффициент, "ЧДЦ=6"),
							КлючУзла));
				КонецЕсли;
			КонецЦикла;
	...
	Для Каждого КлючИЗначение Из СтрокиПоКлючам Цикл
		...
		Если Запись = Неопределено ИЛИ Запись.КлючиУзлов.Количество() = 0 Тогда
			...
			ЗаписьЖурналаРегистрации("СтруктураСебестоимости.НЕРАСПРЕДЕЛЁННЫЕ.ПРОПУСК",
				УровеньЖурналаРегистрации.Предупреждение,,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"Строки без партии в группах: Партия=%1, СтрокПары=%2",
					КлючПартии, Строка(СтрокиПары.Количество())));
			Продолжить;
		КонецЕсли;
		...
		ЗаписьЖурналаРегистрации("СтруктураСебестоимости.НЕРАСПРЕДЕЛЁННЫЕ",
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"Нераспределённые строки: КлючПары=%1, Партия=%2, СтрокПары=%3, Узлов=%4, Коэфф=%5",
				КлючПары, КлючПартии, Строка(СтрокиПары.Количество()),
				Строка(КоличествоУзлов), Формат(Коэффициент, "ЧДЦ=6")));
	...
	Если КоличествоНераспределённых > 0 Тогда
		ЗаписьЖурналаРегистрации("СтруктураСебестоимости.НЕРАСПРЕДЕЛЁННЫЕ.ИТОГО",
			УровеньЖурналаРегистрации.Предупреждение,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"Итого нераспределённых строк: %1", Строка(КоличествоНераспределённых)));
	КонецЕсли;
...
// Формирует SQL-запрос для объединения дополнительных расходов (ПАКЕТНАЯ версия с JOIN на ВТПарыУзлов)
// Используется ТОЛЬКО в СебестоимостьУзловПакетомГруппировка()
// Возвращает: Текст SQL-запроса
Функция ikon_cost_ТекстЗапроса_ТекстДопРасходыОбъединениеПакетный()
...
|		
|ГДЕ 
|	ДетализацияСебестоимости.Номенклатура В (&Продукция)
|	И ДетализацияСебестоимости.Характеристика В (&ХарактеристикиПродукции)
|	И НЕ ДетализацияСебестоимости.ЗатратыПредыдущихПеределов
|	И НЕ ВЫБОР
|			КОГДА &ДанныеПоСебестоимости = 1
...
// Вспомогательная функция для диагностики наличия полей Числитель/Знаменатель в тексте SQL-запроса
// Возвращает структуру с информацией о наличии полей
...
Функция ikon_cost_ТекстЗапросаЗатратыНаПродукцию(ВыводитьДопРасходы)
	...
	//#Вставка
	// Выводим диагностику в журнал регистрации
	ТекстДиагностики = "Диагностика потока данных Числитель/Знаменатель:" + Символы.ПС;
	Для Каждого Диагн Из ДиагностикаВТ Цикл
		ТекстДиагностики = ТекстДиагностики + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"  %1: Числитель=%2, Знаменатель=%3" + Символы.ПС,
			Диагн.ИмяВТ,
			?(Диагн.ЕстьЧислитель, "ЕСТЬ", "НЕТ"),
			?(Диагн.ЕстьЗнаменатель, "ЕСТЬ", "НЕТ"));
	КонецЦикла;
	
	ЗаписьЖурналаРегистрации("СтруктураСебестоимости.Диагностика.Пропорция",
		УровеньЖурналаРегистрации.Предупреждение,
		,
		,
		ТекстДиагностики);
